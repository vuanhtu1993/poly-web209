...
const AddFilmPage = () => {
  ...
  const { data, error, isLoading: isLoadingFecth } = useGetTodosQuery();
  const [addTodo, result] = useAddTodoMutation();
  // result: { isLoading: false, isSuccess: false}
  // result: { isLoading: true, isSuccess: false}
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (formValid.isValidExtract && formValid.isValidTitle) {
      try {
        // 
        await dispatch(createFilm(formData)).unwrap()
        await dispatch(fetchFilms()).unwrap()
        setMessage({
          type: "success",
  ...
  return <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
    ...
    <button onClick={() => addTodo({ name: "todo 1" })}>Click</button>
